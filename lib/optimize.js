...
function jpegoptim(buffer) {
  var tempFilePath = tempfile('.jpg');
  return fsP.writeFile(tempFilePath, buffer)
    .then(() => {
    ...
    .then((buffer1) => {
      return fsP.readFile(tempFilePath).then((buffer) => { fs.unlink(tempFilePath, () => {}); return buffer; })
    })
}
...
function mozjpeg(buffer) {
  ...
  return fsP.writeFile(tempFilePath, buffer)
    ...
    .then((buffer) => {
      return fsP.unlink(tempFilePath).then(() => { return buffer })
    })
}
